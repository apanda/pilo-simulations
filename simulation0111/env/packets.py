...
class ConnectionPacket (Packet):
  ...
  Drop = 2
  Data = 4
  def __init__(self, source, destination, connID, flags, seq, data):
    self.source = source
    self.destination = destination
    self.ID = ConnectionPacket.ID + 1
    ConnectionPacket.ID += 1
    self.connID = connID
    self.flags = falgs
    self.seq = seq
    self.data = data
  ...
  def __hash__ (self):
    return hash((self.connID, self.ID, self.source, self.destination, self.seq, self.flags))

  def __str__ (self):
    return "Conn Packet ID:%d connID:%d seq:%d src:%d dest:%d flags:%d"%(self.ID, self.connID, self.seq, self.source, self.destination, \
        self.flags)

  def pack (self):
    return struct.pack("LL", self.source, self.destination)
